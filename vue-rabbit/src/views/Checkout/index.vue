      ...
      <div class="wrapper">
          ...
          <table class="goods">
            ...
            <tbody>
              <tr v-for="i in checkInfo.goods" :key="i.id">
                ...
                <td>&dollar;{{ Math.round(i.price) }}</td>
                <td>{{ Math.round(i.price) }}</td>
                <td>&dollar;{{ Math.round(i.totalPrice) }}</td>
                <td>&dollar;{{ Math.round(i.totalPayPrice) }}</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div class="box-body">
          <div class="total">
            ...
            <dl>
              <dt>商品總價：</dt>
              <dd>&dollar;{{ checkInfo.summary?.totalPrice.toFixed(2) }}</dd>
            </dl>
            <dl>
              <dt>運<i></i>費：</dt>
              <dd>&dollar;{{ checkInfo.summary?.postFee.toFixed(2) }}</dd>
            </dl>
            <dl>
              <dt>應付總額：</dt>
              <dd class="price">
                &dollar;{{ checkInfo.summary?.totalPayPrice.toFixed(2) }}
              </dd>
            </dl>
          </div>
        </div>
        <!-- 送出訂單 -->
        <div class="submit">
          <el-button type="primary" size="large">送出訂單</el-button>
        </div>
      </div>
    </div>
  </div>
